...
Application::Application(HINSTANCE hInstance)
    ...
    wcscpy_s(m_status1, L"Ðåçóëüòàò Ðîáîòà1: Íå âèêîíàíî");
    wcscpy_s(m_status2, L"Ðåçóëüòàò Ðîáîòà2: Íå âèêîíàíî");
}
...
bool Application::Initialize() {
    ...
    m_hWnd = CreateWindow(L"OOP_WinAppClass", L"ÎÎÏ Ëàáîðàòîðíà 1", WS_OVERLAPPEDWINDOW,
...
void Application::OnCommand(WORD id) {
    ...
    if (id == IDM_WORK1) {
        ...
        if (scrollDlg.Execute()) {
            swprintf_s(m_status1, L"Âèáðàíå ÷èñëî (B1): %d", scrollDlg.GetValue());
        }
        ...
        if (dialog->Execute()) {
            wcscpy_s(m_status2, L"Ðîáîòà2 óñï³øíî çàâåðøåíà");
        }
        else {
            wcscpy_s(m_status2, L"Ðîáîòà2 ñêàñîâàíà");
        }
        InvalidateRect(m_hWnd, NULL, TRUE);
    }
    else if (id == IDM_EXIT) {
